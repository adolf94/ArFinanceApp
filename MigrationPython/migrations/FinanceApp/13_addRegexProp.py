...
def up_migration(db):
    ...
    for row in db["HookMessages"]:
        if row["JsonData"]["action"] == "image_ai_upload":
            print(f"found {row["JsonData"]["imageId"]}")
            data[row["JsonData"]["imageId"]] = row


    for row in db["Files"]:
        if row["id"] in data:
            item = data[row["id"]]
            print(f"found {row["id"]} in Files")
            row["AiData"] = item["ExtractedData"]
            print(json.dumps(row["AiData"]))
    return db
    # return db
