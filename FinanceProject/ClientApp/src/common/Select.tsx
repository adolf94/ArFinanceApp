...
import { Box, Chip, FormControl, FormHelperText, InputLabel, MenuItem, Select as MuiSelect, SelectProps as MuiSelectProps, SelectChangeEvent, Theme, Typography, useTheme } from '@mui/material'
...
function Select<T>(props : SelectProps<T>){
  ...
  const getValue = () => {
    ...
    if (props.multiple) {
      return props.options.filter(e => selected.indexOf(props.getOptionValue(e)) !== -1)
        .map(e => props.getOptionLabel(e)).join(", ");
    } else {
  ...
  const internalOnchange = (key:string) => {
    ...
    const outwardValue = props.multiple ? props.options.filter(opt => newValue.indexOf(props.getOptionValue(opt)) != -1)
      : props.options.find(opt=>newValue.indexOf(props.getOptionValue(opt)) != -1)
  ...
  return <FormControl fullWidth={props.fullWidth}>
    {props.label && <InputLabel sx={{ backgroundColor: 'white', px: 1 }}>
    ...
    <MuiSelect
      ...
      renderValue={() => {
        ...
        return props.options.filter(e => selected.indexOf(props.getOptionValue(e)) !== -1)
          .map(e => <Chip key={props.getOptionValue(e)} size="small" label={props.getOptionLabel(e)}></Chip>);
       }}
      >
      {
        props.options.map((opt) => <MenuItem value={props.getOptionValue(opt)}
