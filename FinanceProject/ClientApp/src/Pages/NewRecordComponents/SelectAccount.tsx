...
import { Box, Dialog, Grid, IconButton, List, ListItem, ListItemButton, Portal, TextField, Typography, useMediaQuery, useTheme } from "@mui/material"
        ...
        &&<>
          ...
          {(accountGroups || []).filter(e => e.accountTypeId == props.typeId).map((e, i) => {
              ...
              {e.name}
                ...
                <Grid item xs={6}>
                    <List>
                        {(accounts || []).filter(e => acctGroup && e.accountGroupId == acctGroup?.id).map(f => <ListItemButton
        ...
        {props.selectType == "vendor"   
            && 
                <Grid item xs={8}>
                  <List>
                  {
                    (filteredVendors || []).map((e,i) => <ListItemButton
  ...
  return <>
    {props.show && !sm && <Box display={{ xs: "none", sm: 'block' }}>
      ...
      <Dialog open={props.show && sm} fullScreen sx={{
          position: "absolute",
          top: "40%"
      }} maxWidth="lg" onClose={props.onClose}>
      {props.selectType !== "calculate" && <SelectAccount {...props} />}
      {props.selectType === "calculate" && <Calculator {...props} />}
    </Dialog>
  </>
