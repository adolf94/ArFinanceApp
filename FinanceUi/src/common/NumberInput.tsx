...
const NumberInput = (props: any) => {
  ...
  useEffect(() => {
    ...
    setValue(numeral((props.value || "0").toString().replace(",", "")).format("0,0.00"))
  }, [props.value])

  const onChange = (evt) => {
    if (!/([0-9,]*\.[0-9]{0,2})*/.test(evt.target.value) && /\.{2,}/.test(evt.target.value)) {
      evt.preventDefault();
      return
    }
  ...
  return <TextField inputProps={{ min: 0, style: { textAlign: 'right' }, ...props.inputProps }} fullWidth variant="standard"
    //InputProps={{
    ...
    onKeyPress={(event) => {
      ...
      if ((value + event.key).split("").filter(e=>e==".").length > 1) event.preventDefault()
    }}
    onBlur={() => {
