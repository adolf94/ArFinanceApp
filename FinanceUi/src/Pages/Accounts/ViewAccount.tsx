import { ArrowLeft, ArrowLeftOutlined, ChevronLeft as IcoChevronLeft, ChevronRight as IcoChevronRight } from "@mui/icons-material"
import { AppBar, Chip, Divider, Grid, IconButton, List, ListItem, Paper, Toolbar, Typography } from "@mui/material"
      ...
      <Grid item md={7} xs={12}>
        <Paper sx={{ my: 1,p:2 }}>
          <Grid container sx={{ textAlign: 'center', justifyContent: 'space-around' }}>
            <Grid item>
              Deposit<br />
              <Typography color="success.main" sx={{ px: 1, alignSelf: 'center' }} variant="transactionHeaderDate">{numeral(data.totals.deposit).format("0,0.00")}</Typography>
            </Grid>
            <Grid item>
              Withdrawal<br />
              <Typography color="danger.main" sx={{ px: 1, alignSelf: 'center' }} variant="transactionHeaderDate">{numeral(data.totals.withdrawal).format("0,0.00")}</Typography>
            </Grid>
            <Grid item>
              Total<br />
              <Typography sx={{ px: 1, alignSelf: 'center' }} variant="transactionHeaderDate">{numeral(data.totals.total).format("0,0.00")}</Typography>
            </Grid>
            <Grid item>
              Balance<br />
              <Typography  sx={{ px: 1, alignSelf: 'center' }} variant="transactionHeaderDate">{numeral(data.totals.total + acctBalance?.balance).format("0,0.00")}</Typography>
            </Grid>
          </Grid>
        </Paper>
        { data.dates.map((data)=> <Paper sx={{ my: 1 }}>
            <List>
              <ListItem dense onClick={() => navigate("/transactions/new?date=" + data.dateGroup + "&creditId=" + acctId)}>
                <Grid item xs={6}>
                ...
                <Typography color="green" sx={{ px: 1, alignSelf: 'center', fontColor: 'success' }} variant="transactionHeaderDate">{numeral(data.income).format("0,0.00")}</Typography>
                </Grid>
                <Grid item xs={3} sx={{ display: 'flex', textAlign: 'center', justifyContent: 'center' }}>
                  <Typography color="red" sx={{ px: 1, alignSelf: 'center', fontColor: 'danger' }} variant="transactionHeaderDate">{numeral(data.expenses).format("0,0.00")}</Typography>
                </Grid>
              </ListItem>
              <Divider />
              {
                data.items.map((item) => <ListItem onClick={() => navigate("../../transactions/" + item.id)}>
                  ...
                  <Grid container>
                    ...
                    <Grid item xs={4} sx={{ textAlign: 'right', px: 1}}>
                      <Typography color={fontColorOnType(item.debitId,item.creditId)} sx={{  fontWeight: 600 }}>P {numeral(item.amount).format("0,0.00")}</Typography>
